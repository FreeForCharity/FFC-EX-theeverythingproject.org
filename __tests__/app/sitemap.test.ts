...
import { metadata as donationMetadata } from '../../src/app/donation/page'
...
const metadataByRoute: Record<string, Metadata> = {
  ...
  '/crayon-drive-photo-album': crayonDrivePhotoAlbumMetadata,
...
describe('sitemap.xml generation', () => {
  ...
  it('should include GitHub Pages base path in route URLs when configured', () => {
    ...
    expect(
      result.find((entry) => entry.url.endsWith('/FFC-EX-theeverythingproject.org/'))
    ).toBeDefined()
    expect(
      result.find((entry) => entry.url.includes('/FFC-EX-theeverythingproject.org/privacy-policy'))
    ).toBeDefined()
  })
